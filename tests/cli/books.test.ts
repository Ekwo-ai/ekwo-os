...
describe('the verbs', () => {
  ...
  it('invoice new writes a draft whose totals are the database\'s', async () => {
    const created = await ekwo([
      'invoice', 'new', '--contact', 'client', '--date', '2026-06-15', '--ref', 'job-7',
  ...
  it('creates once under one --ref, and finishes a creation a dropped connection cut in two', async () => {
    ...
    const again = await ekwo(['invoice', 'new', '--contact', contactId, '--date', '2026-06-15', '--ref', 'job-7', '--line', `name=Audit,price=1500.00,account=${SALES}`]);
    ...
    const finished = await ekwo(['invoice', 'new', '--stdin'], {
  ...
  it('invoice line add puts one more line at the end, found by the reference of the draft', async () => {
    const added = await ekwo(['invoice', 'line', 'add', 'job-7', '--name', 'Travel', '--amount', '250.00', '--account', SALES]);
    expect(added.command).toBe('invoice line add');
  ...
  it('refuses an account that does not exist with code 2, before anything is written', async () => {
    const before = await count('documents');
    const answer = await ekwo(['invoice', 'new', '--contact', contactId, '--line', 'name=Wrong,price=10.00,account=no-such-account']);
    expect(answer.exitCode).toBe(EXIT_USAGE);
    expect(answer.error).toMatchObject({ kind: 'usage', name: 'unknown_account_code' });
    expect(await count('documents')).toBe(before);

    const malformed = await ekwo(['invoice', 'new', '--contact', contactId, '--line', 'Audit 1 500 @21']);
  ...
  it('a locked period is exit code 3, word for word — posting for real and rehearsing alike', async () => {
    const locked = await ekwo(['invoice', 'new', '--contact', contactId, '--date', '2026-03-15', '--ref', 'locked', '--line', `name=Late,price=100.00,account=${SALES}`]);
  ...
  it('match settles a statement line against a document, through settle_from_statement()', async () => {
    await ekwo(['invoice', 'new', '--contact', contactId, '--date', '2026-07-01', '--ref', 'job-9', '--line', `name=Retainer,price=300.00,account=${SALES}`]);
