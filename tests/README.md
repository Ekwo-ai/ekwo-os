...
| `load/load.test.ts` | books at volume — five companies of 10 000 documents, made by copying the golden year the engine posted: that the copy is coherent (every foreign key re-checked, every entry balanced) and deterministic, that statistics exist before a plan is read, and then **the shape of the plan** of the six hot paths, as the owner and as a signed-in member: no sequential scan of a large table, the same answer for both, and `has_capability()` asked a bounded number of times rather than once per row. Times are reported against a budget and never asserted. `docs/load.md` is the long form |
...
| `cli/multi-country.test.ts` | `ekwo init --no-company`, then `ekwo company new` in two countries of the same installation, through `create_company()`; the refusals of `init` (country, chart, language) and of the database (`not_instance_admin`); `ekwo company list`; `init` without the flag unchanged; `ekwo.json` without a country, and an older one that still names one |
