-- Ekwo OS — a supply measured against its seller.
--
-- `20260916125000_a_tax_follows_the_territory.sql` let a tax name the
-- territory each party has to be in, and three cases stayed out of reach.
-- `docs/international.md` records all three and none of them asks for an
-- operator:
--
--   1. **A place that is the seller's own, whichever it is.** California
--      exempts a sale the contract requires to be shipped to a point outside
--      the State (Revenue and Taxation Code, section 6396); a country with
--      state taxes levies one pair of taxes on a supply that stays inside the
--      seller's state and another on one that leaves it. The condition is not
--      "the supply is in X" but "the supply is where the seller is", and a
--      pack that wrote it as fifty `supply_in` would write it wrong fifty
--      times.
...
--
-- The answers, one each, and the argument in `docs/decisions.md`, "A supply
-- measured against its seller":
--
--   1. `taxes.applies_supply_vs_seller` — `same` or `other`. A closed
--      relation between two facts the document already carries, not a
--      negation and not an expression.
--   2. `territories.outside_parent_tax` — reference data, and
--      `territory_within_for_tax()`, which walks the tree and stops at such a
--      territory. A pack that says `supply_in: ES` then refuses a delivery to
--      `ES-CN` without writing a word about the Canary Islands.
--   3. `documents.seller_territory_code`, `buyer_territory_code` and
--      `supply_territory_resolved`, written by `post_document()` and frozen by
--      the guard of a posted document.
--
-- Additive: every new column has a default that reproduces what was there, no
-- pack that names no territory reads any of it, and `territory_within()` is
-- unchanged — it answers a question about geography, which other readers ask.

-- ---------------------------------------------------------------------------
-- 1. A territory outside its parent's tax
-- ---------------------------------------------------------------------------

alter table territories
  add column if not exists outside_parent_tax boolean not null default false;

alter table territories
  drop constraint if exists territories_outside_parent_tax_has_parent;
alter table territories
  add constraint territories_outside_parent_tax_has_parent
    check (not outside_parent_tax or parent_code is not null);

comment on column territories.outside_parent_tax is
  'True where the tax of the parent territory does not apply here, although the territory is part of it: the Canary Islands, Ceuta and Melilla for Spanish VAT, Büsingen and Heligoland for German VAT, Livigno and Campione d''Italia for Italian VAT. Read by territory_within_for_tax(), so a tax conditioned on the parent does not reach a party or a supply here. Independent from eu_vat_scope, which says how far the Union''s common system reaches and is read by ec_sales_list(): Åland is outside the second and inside Finnish VAT. Set by the seed of territories with the text that excludes the territory in legal_reference, never by a pack.';

create or replace function territory_within_for_tax(p_code text, p_of text)
returns boolean
language sql
stable
as $$
  with recursive up as (
    select t.code, t.parent_code, t.outside_parent_tax
      from territories t
    ...
    select t.code, t.parent_code, t.outside_parent_tax
     ...
     where not up.outside_parent_tax
  )
  select exists (select 1 from up where up.code = upper(btrim(p_of)));
$$;

comment on function territory_within_for_tax(text, text) is
  'territory_within(), cut at every territory whose outside_parent_tax is true: the walk up territories.parent_code stops there. So ES-CN is within ES-CN and not within ES, and US-CA is within US as before. What post_document() reads to judge a tax''s territory conditions. False for a code this table does not carry.';

revoke execute on function territory_within_for_tax(text, text) from public, anon;
grant  execute on function territory_within_for_tax(text, text) to authenticated, service_role;

-- ---------------------------------------------------------------------------
-- 2. The place of supply, measured against the seller
--
-- Two values, and the vocabulary is closed for the reason `applies_when` has
-- three keys and no operator: a relation a reviewer reads in a diff, not a
-- condition a pack computes. `same` holds where the supply lies inside the
-- seller's territory, `other` where it does not — both read with
-- `territory_within_for_tax()`, so a Spanish seller's delivery to the Canary
-- Islands is `other`.
-- ---------------------------------------------------------------------------

do $$
begin
  if not exists (select 1 from pg_type where typname = 'territory_relation') then
    create type territory_relation as enum ('same', 'other');
  end if;
end;
$$;

comment on type territory_relation is
  'Where the place of supply lies against the territory of the seller: same (inside it) or other (outside it). A relation between two facts a document carries, never a negation of a territory.';

alter table tax_templates
  add column if not exists applies_supply_vs_seller territory_relation;
alter table taxes
  add column if not exists applies_supply_vs_seller territory_relation;

comment on column tax_templates.applies_supply_vs_seller is
  'Whether the supply has to lie inside the seller''s territory (same) or outside it (other), from `applies_when.supply_vs_seller` of the pack. Null means the tax says nothing about it.';
comment on column taxes.applies_supply_vs_seller is
  'Whether the supply has to lie inside the seller''s territory (same) or outside it (other). Read at the level of the seller: post_document() refuses a document whose seller is known only by a country, and one that contradicts the relation.';

-- ---------------------------------------------------------------------------
-- 3. What the engine believed, kept on the document
--
-- Plain text and no foreign key, on purpose. The values are what
-- `document_territory()` answered, and its last rung is a party's country —
-- `contacts.country`, which is not a key of `territories`. A foreign key here
-- would refuse to post an invoice to a customer in a country no pack books in,
-- which is a refusal nobody asked for and the opposite of a record.
--
-- Null on every document posted before this migration: the answer was not
-- kept then, and reconstructing it now from contacts that may have moved would
-- be recording a belief the engine never held.
-- ---------------------------------------------------------------------------

alter table documents
  add column if not exists seller_territory_code     text,
  add column if not exists buyer_territory_code      text,
  add column if not exists supply_territory_resolved text;

comment on column documents.seller_territory_code is
  'The territory of the seller as post_document() resolved it when the document was posted — the company on a sale, the contact on a purchase, its territory_code or failing that its country. Written by posting, frozen with the document, given back to null when unpost_document() returns it to draft. Null on a draft and on a document posted before it was recorded.';
comment on column documents.buyer_territory_code is
  'The territory of the buyer as post_document() resolved it when the document was posted. Frozen with the document; null on a draft.';
comment on column documents.supply_territory_resolved is
  'The place of supply as post_document() resolved it when the document was posted: supply_territory_code, failing that delivery_country, failing that the buyer''s territory. What the tax''s territory conditions were judged against. Frozen with the document; null on a draft.';

-- ---------------------------------------------------------------------------
-- 4. The copies of a tax, and its comparison
--
-- `20260916125000` as it was, with the fourth condition carried through each.
-- A column missing on one side of a copy is the defect `price_include` was for
-- a whole release.
-- ---------------------------------------------------------------------------
...
begin
  ...
  loop
    ...
    values (p_company_id, r.code,
            ...
            r.applies_supply_territory, r.applies_supply_vs_seller)
    on conflict (company_id, code) do nothing
  ...
  select 'tax', x.code, 'differs', 'review'::pack_change_rule,
         jsonb_build_object(
           ...
           'company', jsonb_build_object('name', x.name, 'amount', x.amount,
                                         ...
                                         'applies_supply_vs_seller', x.applies_supply_vs_seller))
    from taxes x
    join tax_templates t on t.country = v_country and t.code = x.code
   where x.company_id = p_company_id
     and (label_for(t.name, null, preferred_languages(p_company_id)) is distinct from x.name
          ...
          or t.applies_supply_vs_seller is distinct from x.applies_supply_vs_seller);
...
-- ---------------------------------------------------------------------------
-- 5. post_document, which judges the relation and keeps its answer
--
-- `20260916126000` as it was, with three changes: the parties are resolved on
-- every document, the conditions are judged with territory_within_for_tax()
-- and the relation to the seller, and the answer is written beside the entry.
-- ---------------------------------------------------------------------------
...
declare
  ...
  v_tax_point  date;
  v_postings   integer;
  -- Where the three parties of this document are, resolved once and kept on
  -- the document. `document_territory()` is the ladder.
  v_terr_seller text;
  v_terr_buyer  text;
  v_terr_supply text;
  r            record;
...
begin
  ...
  -- Where the three parties of this document are. Resolved on every document
  -- now and not only where a tax asks, because the answer is kept: it is
  -- written on the document beside its entry, at the end of this function,
  -- and a posted document does not change it afterwards.
  ...
  --
  -- "Inside" is `territory_within_for_tax()`: the tree of `territories`, cut
  -- at every territory the reference data takes out of its parent's tax. A
  -- supply to such a territory does not satisfy a condition naming the
  -- parent, which is how a country's rates stop at the territories its own
  -- law excludes without a pack writing a negation.
  --
  -- The loop is skipped entirely where nothing asks, which is every document
  -- of every pack that names no territory.
  for r in
    select distinct t.code,
           t.applies_seller_territory as seller,
           t.applies_buyer_territory  as buyer,
           t.applies_supply_territory as supply,
           t.applies_supply_vs_seller as relation
     ...
     where l.document_id = p_document_id
       and (t.applies_seller_territory is not null
         ...
         or t.applies_supply_vs_seller is not null)
     order by 1
  loop
    for p in
    ...
    loop
      ...
      if not territory_within_for_tax(p.actual, p.wanted) then
    ...
    -- The relation between two facts the document already carries: is the
    -- supply inside the seller's territory, or not. It is read at the level of
    -- the seller — a seller in US-CA shipping to US-AZ is `other`, and one in
    -- US-CA delivering in US-CA is `same` — so a seller known only by a whole
    -- country has no level to read it at. That is refused rather than
    -- answered: a country is where no tax that asks this question is levied,
    -- and "same" against a country would hold for every domestic supply of
    -- every state at once.
    if r.relation is not null then
      if v_terr_seller is null then
        raise exception 'no_party_territory: tax % compares the place of supply with the territory of the seller, and nothing on this document says where the seller is; set territory_code on the party that sells, or the country beside it',
          r.code;
      end if;
      if v_terr_supply is null then
        raise exception 'no_party_territory: tax % compares the place of supply with the territory of the seller, and nothing on this document says where the supply is; set supply_territory_code or delivery_country on the document',
          r.code;
      end if;
      if not exists (select 1 from territories t
                      where t.code = upper(btrim(v_terr_seller)) and t.parent_code is not null) then
        raise exception 'no_party_territory: tax % compares the place of supply with the territory of the seller, and the seller is in %, which is not a territory inside a country; set territory_code on the party that sells to the territory that levies this tax',
          r.code, v_terr_seller;
      end if;
      if territory_within_for_tax(v_terr_supply, v_terr_seller) <> (r.relation = 'same') then
        raise exception 'tax_territory_mismatch: tax % applies where the supply is % the territory of the seller (%); on this document the supply is in %',
          r.code,
          case when r.relation = 'same' then 'inside' else 'outside' end,
          v_terr_seller, v_terr_supply;
      end if;
    end if;
  end loop;
  ...
  update documents
     set state  = 'posted',
         ...
         seller_territory_code     = v_terr_seller,
         buyer_territory_code      = v_terr_buyer,
         supply_territory_resolved = v_terr_supply
...
-- ---------------------------------------------------------------------------
-- 6. unpost_document, which gives the three territories back
--
-- `20260919190727` as it was: what posting derived goes, and the territories
-- are now part of what posting derived.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 7. The guard of a document
--
-- `20260919190727` as it was. The three columns are frozen once a document has
-- left draft without a word here — the list of what may move is closed — and
-- this only lets them go back to null with the rest of what posting derived.
-- ---------------------------------------------------------------------------
...
-- Grants
--
-- One new function, granted above beside its revoke. No new table, so no new
-- policy: the columns sit on tables whose row level security already decides
-- who reads them. The replaced functions keep their grants and comments.
-- ---------------------------------------------------------------------------
