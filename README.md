...
- **Every country a pack, out of the box.** <!-- generated:countries -->United Arab Emirates (`ae`), Austria (`at`), Australia (`au`), Belgium (`be`), Burkina Faso (`bf`), Bénin (`bj`), République démocratique du Congo (`cd`), Centrafrique (`cf`), Congo (`cg`), Schweiz (`ch`), Côte d’Ivoire (`ci`), Cameroun (`cm`), Czechia (`cz`), Germany (`de`), Danmark (`dk`), Estonia (`ee`), España (`es`), Finland (`fi`), France (`fr`), Gabon (`ga`), United Kingdom (`gb`), Guinée (`gn`), Guinée équatoriale (`gq`), Guinée-Bissau (`gw`), Hong Kong (`hk`), Magyarország (`hu`), Ireland (`ie`), Italia (`it`), 日本 (`jp`), Comores (`km`), 대한민국 (`kr`), Luxembourg (`lu`), Mali (`ml`), México (`mx`), Niger (`ne`), Nederland (`nl`), Norge (`no`), New Zealand (`nz`), Polska (`pl`), Portugal (`pt`), Saudi Arabia (`sa`), Sverige (`se`), Singapore (`sg`), Sénégal (`sn`), Tchad (`td`), Togo (`tg`), ประเทศไทย (`th`), 臺灣 (`tw`), United States (`us`) and Việt Nam (`vn`)<!-- /generated --> — each with its chart of accounts, its tax codes, its
