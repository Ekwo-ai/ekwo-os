...
- **Every country a pack, out of the box.** <!-- generated:countries -->Australia (`au`), Belgium (`be`), Bénin (`bj`), Côte d’Ivoire (`ci`), Cameroun (`cm`), Germany (`de`), Estonia (`ee`), España (`es`), France (`fr`), United Kingdom (`gb`), Ireland (`ie`), Luxembourg (`lu`), México (`mx`), Niger (`ne`), Nederland (`nl`), Singapore (`sg`), Sénégal (`sn`) and United States (`us`)<!-- /generated --> — each with its chart of accounts, its tax codes, its
