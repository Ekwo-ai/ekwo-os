...
- **Every country a pack, out of the box.** <!-- generated:countries -->Belgium (`be`), Côte d’Ivoire (`ci`), Germany (`de`), Estonia (`ee`), España (`es`), France (`fr`), United Kingdom (`gb`), Ireland (`ie`), Luxembourg (`lu`), México (`mx`), Nederland (`nl`), Sénégal (`sn`) and United States (`us`)<!-- /generated --> — each with its chart of accounts, its tax codes, its
