...
`packs/pe/`, `community`, seed 96, the second pack of Latin America after
Mexico. Its value added tax is the Impuesto General a las Ventas, TUO
approved by Decreto Supremo N.° 055-99-EF; its comprobante de pago
electrónico regime is the Sistema de Emisión Electrónica of the Reglamento de
Comprobantes de Pago (Resolución de Superintendencia N.° 007-99/SUNAT) and its
designating resolutions. Thirteen texts in the register. What follows is what
the core could not say, in addition to the clearance-invoicing gap Mexico
already named.

**One rate, two taxes, no way to stack them.** The 18 % a comprobante prints
is the sum of two different taxes — the IGV of art. 17° of its own Act, and
the Impuesto de Promoción Municipal of art. 76° of the Ley de Tributación
Municipal, a different law with a different beneficiary (the FONCOMUN) —
levied on the same operations under the same rules. `taxes.json`'s `group`
field is reserved for exactly this shape ("the country that stacks two taxes
on one line") and the core does not read it yet: `packs/pe/` declares the
combined figure as one tax, which matches how SUNAT prints it anyway, and
notes the two laws in one `legal_reference`. A country whose comprobante
*did* print the two separately would have no way to say so.

**A rate that moves every year without a new law.** Ley N.° 32387 does not
set a new IGV rate: it sets a five-year schedule, 2026 to 2029, that a reader
has to apply for themselves. A tax code's `valid_from`/`valid_to` pair states
one rate for one span, so this pack opens 1 January 2026 at 18 % (15.5 % +
2.5 %) and will need a new code, and a `valid_to` on this one, on 1 January
2027 — four codes over four years for a rate that, read as one figure, never
changes. Nothing here is a defect of the code shape; it is worth naming
because the *composition* the law actually amends is invisible in
`tax_report.json` and in every figure this pack posts, which state 18 %
throughout and are right to.

**A deadline that shifts by the last digit of a RUC.** The same shape Mexico
met with its own sixth-digit rule: SUNAT's yearly cronograma staggers the
monthly due date across roughly ten working days by the taxpayer's own
registration number, and `deadline.rule: depends_on_taxpayer` is the correct
and complete answer this format already has for it — nothing to fix, an
instance of an existing gap worth recording for the next country that meets
its own digit-based calendar.

**No box for a credit carried from a prior period.** The real Formulario
Virtual N.° 621 restates, inside the declaration itself, the credit balance
of the previous month (casillas 145/184) before it nets the period's own
result. `packs/pe/` declares only casilla 140, the period's own result, which
is what `401113`/`401114` already carry on the ledger from one month to the
next: the boxes that only restate a ledger balance already carried forward
are not declared, the way Belgium's `LU-…` sections are not re-derived where
a total already covers them. The distinction is the same one *A base is
written once* makes for a tax; there is no gap in the core to record, only a
reader's expectation of a box that mirrors the account rather than adding
to it.

**Detracciones, percepciones and retenciones of the IGV, and the Amazonía and
IVAP regimes, are not a rate or an exemption.** They are separate
administrative collection mechanisms — a percentage a buyer deposits into
the seller's own detraction account at the Banco de la Nación, an amount a
designated agent withholds or collects on behalf of SUNAT, a reduced-rate
regime tied to a region defined by a list of districts, and a distinct tax on
milled rice — each with its own accounts, its own casillas (168, 171, 176,
179, 326, 327…) and, for detracciones, its own bank account per taxpayer that
the core has no column for. None of them is a `tax` this format's three
posting types can express, and none is invented here: they are named as a
gap and left for a module or a later version of this pack, the way Mexico
left its DIOT and its *IVA retenciones* declaration.
