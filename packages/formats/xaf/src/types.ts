...
export interface ReadOptions {
  /**
   * How bytes are decoded. An XML file says it in its declaration, and that is
   * what is read: UTF-8 when it says nothing, as XML itself provides. Given
   * here, it has to agree with the declaration, or the file is refused — the
   * caller and the file disagreeing is not settled by picking one.
   */
  encoding?: Encoding;
  /** Refused beyond this many bytes, before anything is read. Default 256 MiB. */
  maxBytes?: number;
  /** Refused beyond this many elements, counted while reading. Default 20 million. */
  maxElements?: number;
}
