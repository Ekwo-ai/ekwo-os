...
/** The two encodings the FEC may be written in, and nothing else. */
export type Encoding = 'utf-8' | 'iso-8859-15';

export interface ReadOptions {
  /**
   * How bytes are decoded. Default `utf-8`, fatally: bytes that are not UTF-8
   * are refused by name, never read as something else. A file written in
   * ISO 8859-15 is said to be so, because every sequence of bytes is valid
   * ISO 8859-15 and a guess would always succeed.
   */
  encoding?: Encoding;
  /** Refused beyond this many bytes, before anything is read. Default 64 MiB. */
  maxBytes?: number;
}
