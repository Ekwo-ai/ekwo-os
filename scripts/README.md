...
| `ohada-packs.mjs` | Copies the chart, the journals, the roles and the two statements the OHADA member States share from `packs/ohada/` into each member pack (`--write`), and without the flag refuses a member whose copy has drifted. `packs/ohada/README.md` is how a country is added | yes — the check |
